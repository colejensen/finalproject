...
next_tree, meta = bt.loadJSON(nx_tree, json_translation, json_meta) # give loadJSON the name of the tree file, the translation dictionary and (optionally) the meta file
...
for l in ll.Objects:
    ...
    if kc != kpc:
        ...
        try:
            ...
            #print(subtree)
            subtree.traverse_tree(k, traverse_condition=traverse_condition, reset_branches=False)
            ...
            if kpc == 'New York City':
                print(kpc + ' -> ' + kc )
        except:
            #print('SKIPPED:' + kpc + ' > ' + kc)
            pass
        #print(fields)
...
cu_func = lambda k: 'k' # means it's black
zu_func = lambda k: 99
#
#
#
...
#
         ...
         loc_tree.plotPoints(ax, x_attr=x_attr, y_attr=y_attr, size=su_func, colour=cu_func, zorder=99)
#
